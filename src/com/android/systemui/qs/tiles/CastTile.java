...
public class CastTile extends QSTileImpl<BooleanState> {
    ...
    /// M: Remove cast tile when WFD disable @{
    @Override
    public boolean isAvailable() {
        return mController.isWfdSupported();
    }
    /// @}
    ...
    private final class Callback implements CastController.Callback, KeyguardMonitor.Callback {
        ...
        public void onWfdStatusChanged(WifiDisplayStatus status, boolean sinkMode) {
            ...
            mUiHandler.post(() -> mDetailAdapter.wfdStatusChanged(status, sinkMode));
            refreshState();
        }
        ...
        public void onWifiP2pDeviceChanged(WifiP2pDevice device) {
            ...
            mUiHandler.post(() -> mDetailAdapter.updateDeviceName(device));
        }
        /// @}
    };
    ...
    private final class CastDetailAdapter implements DetailAdapter, QSDetailItems.Callback {
            ...
            if (mController.isWfdSinkSupported()) {
                        ...
                        public void onClick(View v) {
                            ...
                            if (!checked) {
                                mActivityStarter.postStartActivityDismissingKeyguard(
                                            new Intent(WFD_SINK_SETTINGS), 0);
                            }
                ...
                if (convertView instanceof LinearLayout && mDetailView != null) {
                    ...
                    devider.setLayoutParams(new ViewGroup.LayoutParams(
                        ViewGroup.LayoutParams.MATCH_PARENT, dh));
        ...
        private void wfdStatusChanged(WifiDisplayStatus status, boolean sinkMode) {
            ...
            if (DEBUG) Log.d(TAG, "wfdStatusChanged show " + show);
        ...
        private void handleWfdStateChanged(int wfdState, boolean sinkMode) {
            if (DEBUG) Log.d(TAG, "handleWfdStateChanged wfdState: " + wfdState
                    + " ,sinkMode: " + sinkMode);
            switch (wfdState) {
