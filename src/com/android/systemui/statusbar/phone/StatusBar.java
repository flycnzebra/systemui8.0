...
//        return mStatusBarView.findViewById(R.id.recent_apps007);
    ...
    private void setRecentsButton() {
        if (mStatusBarView != null
                && getStatusBarViewRecentsButton() != null) {
    ...
    private void updateNotificationShade() {
        ...
        for (int i = 0; i < N; i++) {
            ...
            String packName = ent.notification.getPackageName();

            FlyLog.d("Notification packageName=" + packName);

            //@FlyZebra 禁用应用在通知栏显示
            if ("android".equals(packName) ||
                "com.jancar.settingss".equals(packName)) {
                FlyLog.e("Do not show Notification, packname=%s", packName);
                continue;
            }
    ...
    public void addQsTile(ComponentName tile) {
        if (mQSPanel == null) {
            Log.w(TAG, "addQsTile fail not qspanel, componentname: " + tile);
            return;
        }
    ...
    public void remQsTile(ComponentName tile) {
        if (mQSPanel == null) {
            Log.w(TAG, "remQsTile fail not qspanel, componentname: " + tile);
            return;
        }
    ...
    public void clickTile(ComponentName tile) {
        if (mQSPanel == null) {
            Log.w(TAG, "clickTile fail not qspanel, componentname: " + tile);
            return;
        }
...
//            }
    ...
    boolean panelsEnabled() {
        if (atcEnhancementSupport()) {
            if (ActivityManager.isUserAMonkey()) {
                Log.d(TAG, "monkey testing, disable Panels");
                return false;
            }
            boolean hasPinnedHeadUp = false;

            hasPinnedHeadUp = ((mHeadsUpManager != null) && mHeadsUpManager.hasPinnedHeadsUp());
            return !ONLY_CORE_APPS
                    && (hasPinnedHeadUp || ((mDisabled1 & StatusBarManager.DISABLE_EXPAND) == 0));
        } else {
    ...
    public void animateExpandSettingsPanel(String subPanel) {
        ...
        if (subPanel != null) {
            if (mQSPanel == null) {
                Log.w(TAG, "animateExpandSettingsPanel fail not qspanel, subPanel: " + subPanel);
                return;
            }
...
///@}
    ...
    void checkBarMode(int mode, int windowState, BarTransitions transitions) {
        ...
        /*final*/
        boolean anim = !mNoAnimationOnNextBarModeChange && mDeviceInteractive
    ...
    private BroadcastReceiver mBroadcastReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (Intent.ACTION_CLOSE_SYSTEM_DIALOGS.equals(action)) {
                ...
                if (mQSPanel == null) {
                    Log.w(TAG, "mBroadcastReceiver fail not qspanel, action: " + action);
                    return;
                }
...
//                                    btn_back.setVisibility(View.VISIBLE);
    ...
    protected boolean shouldPeek(Entry entry, StatusBarNotification sbn) {
        ...
        if (mNotificationData.shouldFilterOut(sbn)) {
            if (true/**DEBUG*/) Log.d(TAG, "No peeking: filtered notification: " + sbn.getKey());
            return false;
        }
        ...
        if (!isDozing() && mNotificationData.shouldSuppressScreenOn(sbn.getKey())) {
            if (true/**DEBUG*/) Log.d(TAG, "No peeking: suppressed by DND: " + sbn.getKey());
            return false;
        }
        ...
        if (entry.hasJustLaunchedFullScreenIntent()) {
            if (true/**DEBUG*/) Log.d(TAG, "No peeking: recent fullscreen: " + sbn.getKey());
            return false;
        }

        if (isSnoozedPackage(sbn)) {
            if (true/**DEBUG*/) Log.d(TAG, "No peeking: snoozed package: " + sbn.getKey());
            return false;
        }
        ...
        /// M: Add this log for heads up notification debugging.
