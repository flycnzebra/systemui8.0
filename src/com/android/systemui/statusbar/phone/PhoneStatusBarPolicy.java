...
public class PhoneStatusBarPolicy implements Callback, Callbacks,
    ...
    private final String mSlotEmbms;
    ...
    protected void updateEmbmsState(Intent intent) {
        int isActive = intent.getIntExtra(MtkTelephonyIntents.EXTRA_IS_ACTIVE, 0);
        if (DEBUG) Log.d(TAG, "updateEmbmsState  active = " + isActive);
        if (isActive == 1) {
            mIconController.setIcon(mSlotEmbms, R.drawable.stat_sys_embms,
                    mContext.getString(R.string.accessibility_embms_enabled));
            mIconController.setIconVisibility(mSlotEmbms, true);
        } else {
            mIconController.setIconVisibility(mSlotEmbms, false);
        }
    }
    ...
    /*private*/ BroadcastReceiver mIntentReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            } else if (action.equals(MtkTelephonyIntents.ACTION_EMBMS_SESSION_STATUS_CHANGED)) {
