...
public class DarkIconDispatcherImpl implements DarkIconDispatcher {
    ...
    public void addDarkReceiver(ImageView imageView) {
        //ATC{ add imageView.setColorFilter(mIconTint);
        DarkReceiver receiver;
        if (android.os.SystemProperties.getBoolean("ro.atc.aosp_enhancement", false)) {
            receiver = new DarkReceiver() {
                public void onDarkChanged(Rect area, float darkIntensity, int tint) {
                    imageView.setImageTintList(
                        ColorStateList.valueOf(getTint(mTintArea, imageView, mIconTint)));
                    imageView.setColorFilter(mIconTint);
                }
            };
        } else {
            receiver = (area, darkIntensity, tint) -> imageView.setImageTintList(
                    ColorStateList.valueOf(getTint(mTintArea, imageView, mIconTint)));
        }
        //ATC}
