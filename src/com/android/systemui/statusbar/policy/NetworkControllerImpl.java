...
public class NetworkControllerImpl extends BroadcastReceiver
    ...
    NetworkControllerImpl(Context context, ConnectivityManager connectivityManager,
        ...
        mStatusBarSystemUIExt = OpSystemUICustomizationFactoryBase.getOpFactory(context)
    ...
    private void addCustomizedAction(IntentFilter filter) {
      filter.addAction(TelephonyIntents.ACTION_SUBINFO_RECORD_UPDATED);
        /// Add for 4G+W.
        filter.addAction(NetworkTypeUtils.LWA_STATE_CHANGE_ACTION);
    }
    /// @ }
    ...
    private void updateMobileControllersEx(Intent intent){
        int detectedType = MtkSubscriptionManager.EXTRA_VALUE_NOCHANGE;
        if (intent != null) {
            detectedType = intent.getIntExtra(MtkSubscriptionManager.INTENT_KEY_DETECT_STATUS, 0);
        ...
        if (detectedType != MtkSubscriptionManager.EXTRA_VALUE_REPOSITION_SIM) {
    ...
    void setCurrentSubscriptions(List<SubscriptionInfo> subscriptions) {
        ...
        for (int i = 0; i < num; i++) {
            ...
            if (cachedControllers.indexOfKey(subId) >= 0 &&
                    /// M: Fix ALPS03475067,when sim exchanged. ims listener need to register.
                    cachedControllers.get(subId).mSubscriptionInfo.getSimSlotIndex()
                        == subscriptions.get(i).getSimSlotIndex()) {
                /// M: Fix bug ALPS02416794 @{
                MobileSignalController msc = cachedControllers.get(subId);
    ...
    private int getSubIdFromNetrequest(NetworkSpecifier networkspecifier) {
        int subId = mSubscriptionManager.INVALID_SUBSCRIPTION_ID;
        if (networkspecifier instanceof StringNetworkSpecifier) {
            try {
                subId = Integer.parseInt(((StringNetworkSpecifier) networkspecifier).specifier);
            } catch (NumberFormatException e) {
                Log.e(TAG, "NumberFormatException on ");
            }
        }
        return subId;
    }
    ...
    private void handleLwaAction(Intent intent) {
        int phoneId = intent.getIntExtra(NetworkTypeUtils.EXTRA_PHONE_ID, -1);
        int lwaRegState = intent.getIntExtra(
                NetworkTypeUtils.EXTRA_STATE, NetworkTypeUtils.LWA_STATE_UNKNOWN);
        Log.d(TAG, "onRecevie ACTION_LWA:" + phoneId + ",lwa: state" + lwaRegState);
        for (int i = 0; i < mMobileSignalControllers.size(); i++) {
            MobileSignalController controller = mMobileSignalControllers.valueAt(i);
            if (controller.getControllerSubInfo().getSimSlotIndex() == phoneId) {
                controller.handleBroadcast(intent);
                break;
            }
        }
    ...
    public void refreshPlmnCarrierLabel() {
        for (int i = 0; i < mSlotCount; i++) {
            boolean found = false;
            for (int j = 0; j < mMobileSignalControllers.size(); j++) {
                MobileSignalController controller = mMobileSignalControllers.valueAt(j);
                int slotId = -1;
                ...
                if (i == slotId) {
                    ...
                    mStatusBarPlmnPlugin.updateCarrierLabel(i, true,
            ...
            if (!found) {
                ...
                mStatusBarPlmnPlugin.updateCarrierLabel(i, false, false, mNetworkName);
